...
#error "Unsupported number of Q blocks"
   ...
   fprintf(stderr,
           "\n |                                                           |");
   fprintf(stderr,"\n | Maximum plaintext bitstream length:............%5d (b)  |",
           KOBARA_IMAI_MAX_PTX_BIT_LENGTH);
   fprintf(stderr,"\n | Maximum plaintext bytestream length:...........%5d (B)  |",
           KOBARA_IMAI_MAX_PTX_BYTE_LENGTH);
   fprintf(stderr,
           "\n | KI-g CONSTANT format: 1000---01                           |");
   fprintf(stderr,"\n | Max bitsize encodable w/ constant weight:......%5d (b)  |",
           MAX_ENCODABLE_BIT_SIZE_CW_ENCODING);
   fprintf(stderr,"\n | Bitsize of data encoded w/ constant weight:....%5d (b)  |",
   ...
   fprintf(stderr,
           "\n KEYSIZETABLE: %d/%d & %5lu & %5lu & %5lu & %5d & %5d \\\\ \n",
           CATEGORY, N0,
           sizeof(privateKeyMcEliece_t),
           (N0*(DV+M)+ DV*M)*sizeof(uint32_t), /* expanded private */
           sizeof(publicKeyMcEliece_t),
           KOBARA_IMAI_MAX_PTX_BIT_LENGTH/8, /* max plain size */
           N0*NUM_DIGITS_GF2X_ELEMENT*DIGIT_SIZE_B); /* cipher size */
                                 ...
                                 long unsigned int NumTests)
{
   ...
   unsigned char *ct= calloc(1, CRYPTO_BYTES);
   ...
   for (unsigned trials = 0; trials < NumTests; trials++) {
      ...
      ok        = ( memcmp(plaintext,plaintextDec,mlen) == 0 );
      memcmpOk += ok;
